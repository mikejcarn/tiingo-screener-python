...
#         show_valleys = valleys
#  
#     # FIX 1: Check if we need peaks/valleys for averages even if not displaying
#     need_peaks_valleys_for_avg = (peaks_valleys_avg or peaks_avg or valleys_avg or All_avg)
#  
#     # If nothing is requested, return empty
#     if not (show_peaks or show_valleys or gaps or OB or BoS_CHoCH or need_peaks_valleys_for_avg):
#         return {}
#
#     # -------------------------
#     # Process peaks and valleys params
...
#     if peaks_params is not None:
#         peaks_configs = ensure_config_list(peaks_params, default_config)
#     elif (show_peaks or need_peaks_valleys_for_avg) and peaks_valleys_params is not None:
#         # Fall back to legacy params if available
#         peaks_configs = ensure_config_list(peaks_valleys_params, default_config)
#     elif show_peaks or need_peaks_valleys_for_avg:
#         # Default config
#         peaks_configs = [default_config.copy()]
#     else:
#         peaks_configs = []
...
#     if valleys_params is not None:
#         valleys_configs = ensure_config_list(valleys_params, default_config)
#     elif (show_valleys or need_peaks_valleys_for_avg) and peaks_valleys_params is not None:
#         # Fall back to legacy params if available
#         valleys_configs = ensure_config_list(peaks_valleys_params, default_config)
#     elif show_valleys or need_peaks_valleys_for_avg:
#         # Default config
#         valleys_configs = [default_config.copy()]
#     else:
#         valleys_configs = []
#
#     # FIX 2: Ensure we have at least one config if we need averages
#     if need_peaks_valleys_for_avg and not peaks_configs and not valleys_configs:
#         peaks_configs = [default_config.copy()]
#         valleys_configs = [default_config.copy()]
#
#     # -------------------------
...
#             all_individual_aVWAPs.update(config_peaks)
#
#         # Process valleys for each config
#         for config_idx, config in enumerate(valleys_configs):
#             max_aVWAPs = config.get('max_aVWAPs', None)
#           
#             # Apply channel filtering if needed
...
#     if peaks_valleys or show_peaks or show_valleys:
#         for key, value in all_individual_aVWAPs.items():
#             if 'aVWAP_peak_c' in key or 'aVWAP_valley_c' in key:
#                 df[key] = value
#   
...
#                 lookback = get_lookback(gaps_configs[config_idx], 'avg_lookback', avg_lookback)
#             elif config_idx < len(OB_configs):
#                 lookback = get_lookback(OB_configs[config_idx], 'avg_lookback', avg_lookback)
#             elif config_idx < len(BoS_CHoCH_configs):
#                 lookback = get_lookback(BoS_CHoCH_configs[config_idx], 'avg_lookback', avg_lookback)
#           
#             avg_name = 'All_avg' if config_idx == 0 else f'All_avg_{config_idx}'
#             df[avg_name] = calculate_rolling_aVWAP_avg(df, all_individual_aVWAPs, lookback)
#
#     # Format output
#     cols_to_drop = ['Open', 'Close', 'High', 'Low', 'Volume']
#     if peaks_valleys or show_peaks or show_valleys or peaks_valleys_avg or peaks_avg or valleys_avg or All_avg:
#         cols_to_drop.extend(['Valleys', 'Peaks'])
#     if gaps or gaps_avg or All_avg:
#         cols_to_drop.extend(['Gap_Up', 'Gap_Down'])
#     if not keep_OB_column:
...
#     OB_configs = ensure_config_list(OB_params, {
#         'periods': 25,
#         'max_aVWAPs': None,
#         'include_bullish': True,
#         'include_bearish': True
...
#     peaks_only_aVWAPs = {}
#     valleys_only_aVWAPs = {}
#     peaks_valleys_aVWAPs = {}  # NEW: For combined
#     gaps_aVWAPs = {}
#     OB_aVWAPs = {}
#     BoS_CHoCH_aVWAPs = {}
#
...
#
#     # =====================
#     # Process PEAKS ONLY (using peaks_configs)
#     # =====================
...
#
#     # =====================
#     # Process VALLEYS ONLY (using valleys_configs)
#     # =====================
...
#
#     # =====================
#     # Process COMBINED PEAKS+VALLEYS (using peaks_valleys_configs)
#     # =====================
...
#
#     # =====================
#     # Process other types (gaps, OB, BoS/CHoCH) - unchanged
#     # =====================
#     if 'gaps' in aVWAP_anchors:
#         base_gap_up_indices = df[df['Gap_Up'] == 1].index.tolist() if 'Gap_Up' in df.columns else []
#         base_gap_down_indices = df[df['Gap_Down'] == 1].index.tolist() if 'Gap_Down' in df.columns else []
...
#     keep_OB_column=False,
#     aVWAP_channel=False,
#     deduplicate=True  # NEW: Enable deduplication
# ):
#     """
#     Calculate anchored VWAP channels from market structure points.
#    
#     NEW PARAMETER:
#         deduplicate: If True, removes duplicate aVWAP columns that contain identical data
#     """
#  
#     # -------------------------
#     # Helpers
...
#         return out
#
#     # NEW: Function to deduplicate columns based on their data
#     def deduplicate_columns(df_in, prefix_patterns):
#         """Remove duplicate columns that contain identical data"""
#         if not deduplicate:
#             return df_in
#            
#         df_out = df_in.copy()
#        
#         # Group columns by their base name (without config index)
#         column_groups = {}
#        
#         for col in df_out.columns:
#             # Check if column matches any prefix pattern
#             for prefix in prefix_patterns:
#                 if col.startswith(prefix):
#                     # Extract base name (remove config index)
#                     parts = col.split('_')
#                     if 'c' in col and '_c' in col:
#                         # Find where config index is
#                         for i, part in enumerate(parts):
#                             if part.startswith('c') and part[1:].isdigit():
#                                 # Reconstruct without config index
#                                 base_name = '_'.join(parts[:i] + parts[i+1:])
#                                 break
#                         else:
#                             base_name = col
#                     else:
...
#                     break
#        
#         # For each group, keep only unique columns
#         for base_name, group_cols in column_groups.items():
#             if len(group_cols) <= 1:
#                 continue
#                
#             # Group columns by their actual data (as tuples)
#             data_groups = {}
#             for col in group_cols:
#                 # Create a tuple of the data (excluding NaN for comparison)
#                 data_tuple = tuple(df_out[col].fillna(-999).values)
#                 if data_tuple not in data_groups:
#                     data_groups[data_tuple] = []
#                 data_groups[data_tuple].append(col)
#            
#             # For each unique data group, keep the first column, drop the rest
#             for data_tuple, cols in data_groups.items():
#                 if len(cols) > 1:
#                     keep_col = cols[0]
#                     drop_cols = cols[1:]
#                     print(f"  Deduplicating: {drop_cols} (identical to {keep_col})")
...
#     # =====================
#     if deduplicate:
#         # Define patterns for aVWAP columns
#         aVWAP_patterns = [
#             'aVWAP_peak_c',
#             'aVWAP_valley_c',
#             'Gap_Up_aVWAP_c',
...
#             'aVWAP_OB_bull_c',
#             'aVWAP_OB_bear_c',
#             'aVWAP_BoS_CHoCH_bull_c',
#             'aVWAP_BoS_CHoCH_bear_c'
#         ]
#         df = deduplicate_columns(df, aVWAP_patterns)
#
#     # -------------------------
#     # Format output
#     # -------------------------
...
#         - 'max_aVWAPs': int or None (default None) - maximum number of aVWAPs to keep
#         - 'mode': str (default 'combined') - for OB: 'bullish', 'bearish', or 'combined'
#         - 'avg_lookback': int (optional) - lookback for averages
#     """
# 
#     # -------------------------
...
#         'periods': 25,
#         'max_aVWAPs': None,
#         'mode': 'combined'  # 'bullish', 'bearish', or 'combined'
#     }
...
#
#     # =====================
#     # Process OB - WITH DEDUP - USING MODE PARAMETER
#     # =====================
#     if 'OB' in aVWAP_anchors:
...
#             ob_col = f'OB_c{config_idx}'
#             if ob_col in df.columns:
#                 # Determine which signals to include based on mode
#                 include_bullish = mode in ['bullish', 'combined']
#                 include_bearish = mode in ['bearish', 'combined']
#                
#                 if aVWAP_channel:
#                     if lowest_valley_idx is not None and include_bullish:
