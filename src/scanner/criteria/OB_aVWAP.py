...
def OB_aVWAP(
    ...
    # Find all aVWAP columns for this mode
    avwap_cols = [col for col in df.columns if f'aVWAP_OB_{mode}_' in col]
    ...
    for i in range(len(scan_range)-1, -1, -1):
        if scan_range['OB'].iloc[i] == target_ob:
            ...
            # Find any aVWAP column that ends with this index
            matching_cols = [col for col in avwap_cols if col.endswith(f'_{i}')]
           
            for avwap_col in matching_cols:
                if pd.notna(latest[avwap_col]):
                    current_avwap = latest[avwap_col]
                    distance = (latest['Close'] - current_avwap) / current_avwap * 100
                    ...
                    if condition and in_range:
                        # Extract config index from column name for info
                        config_part = avwap_col.split('_c')[1].split('_')[0] if '_c' in avwap_col else '0'
